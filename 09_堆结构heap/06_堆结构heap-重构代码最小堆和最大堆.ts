class Heap<T>{
    ...
    constructor(arr:T[],isMax = true) {
        this.isMax = isMax // 这行代码和下面代码不能互换，否则报错，因为下面的return则isMax则未定义导致报错
        if(!arr.length) return
        this.buildHeap(arr)
    }

    // 设计思想:寻求第三方
    private compare(i:number,i2:number):boolean {
        let bolnVal =  this.isMax ? this.data[i] < this.data[i2] : this.data[i] > this.data[i2]
        return bolnVal
    }
    ...
    heapify_down(i:number) {
        ...
        while(2*index+1<this.length){
            ...
            // 你漏了这一步
            if (this.compare(largerIndex,index)) {
                break
            }
