...
class linkList<T> {
    ...
    inserted(element:T,position:number):boolean {
        ...
        if(position === 0) {
            ...
            newNode.Next = previous?.Next?.Next ?? null // 这里需要？？null是因为previous要是为null，则previous?.Next为undefined，则会导致报错，所以？？null意思是当previous?.Next为undefined时，则转为null
        }
        this.size++
        return true
    }
...
console.log(node1.indexof("caomzsya"));
